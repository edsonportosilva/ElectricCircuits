...
#
# em que $i_{L}(t)_{particular}$ é uma solução particular para a EDO (\ref{EDO1}), que corresponde à solução de regime estacionário, ou seja, $i_{L}(t)_{particular} = \lim_{t\to \infty} i_L(t)$. No caso de circuitos de corrente contínua, $i_{L}(t)_{particular}$ é uma função constante sempre que a corrente no indutor tender a se estabilizar num valor constante.
